...
export default function AnimatedPropertySections() {
  ...
  return <>
    <section className="section pale featured-section"><div className="decor-circle circle-one"/><div className="decor-circle circle-two"/><div className="shell section-layer">
      ...
      <div className="cards">{plots.map((plot, i) => <article className="property-card animated-card reveal-up" data-reveal style={{transitionDelay:`${i*140}ms`}} key={plot.size}><div className={`property-image property-${i+1}`} style={{backgroundImage:"url('/images/p1.jpg')"}}><span>For Sale</span><div className="image-shade"/><Link href="/properties" className="quick-view">Quick View</Link></div><div className="card-body"><small>RESIDENTIAL PLOTS</small><h3>{plot.label}</h3><p className="price">{plot.price} <small>onwards</small></p><div className="meta"><span>◫ {plot.size}</span><span>Dholera, Gujarat</span></div><Link href="/contact-us" className="btn small">Get Details <b>→</b></Link></div></article>)}</div>
    </div></section>

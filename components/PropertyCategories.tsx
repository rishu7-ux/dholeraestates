...
const categories = [
  { title: "Dholera Estates", description: "Verified residential plots in Aakru village. Since 2024, helping investors find trusted, transparent land opportunities in Dholera Smart City under the DMIC corridor.", icon: FaHouse, accent: "#ff5c16", soft: "#fff0e8", position: "object-center", image: "/images/dholera-estates-project.png", href: "/properties/dholera-estates" },
  ...
  { title: "Dholera Estates 2", description: "One Dholera, so many cities. Premium 170–350 sq. yd. residential plots in Vallinda village, near Dholera International Airport. Explore Dholera Estates 2 today.", icon: FaIndustry, accent: "#087cf4", soft: "#e8f2ff", position: "object-center", image: "/images/dholera-estates-2-project.png", href: "https://dholeraestate2.com/" },
          ...
          {categories.map((category, index) => {
            ...
            return (
              <motion.article key={category.title} initial={{ opacity: 0, y: 26 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.15 }} transition={{ duration: 0.5, delay: index * 0.08 }} whileHover={{ y: -6 }} className="premium-card-sheen group relative flex flex-col overflow-hidden rounded-[20px] border bg-white/95 shadow-[0_8px_20px_rgba(33,47,80,0.09)] sm:min-h-[290px] sm:flex-row sm:rounded-[22px] sm:shadow-[0_10px_24px_rgba(33,47,80,0.09)]" style={{ borderColor: `${category.accent}55`, borderTopWidth: 5 }}>
                ...
                <div className="flex flex-1 flex-col p-4 sm:w-1/2 sm:p-5"><span className="flex h-11 w-11 items-center justify-center rounded-[14px] text-lg text-white shadow-[0_5px_14px_rgba(17,17,17,0.18)] sm:h-12 sm:w-12 sm:rounded-[15px] sm:text-xl" style={{ background: category.accent }}><Icon aria-hidden="true" /></span><h3 className="mt-2.5 text-lg font-extrabold tracking-[-0.04em] text-[#101827] sm:mt-3 sm:text-xl">{category.title}</h3><p className="mt-1.5 text-[13px] leading-5 text-[#61708a] sm:mt-2 sm:text-sm">{category.description}</p><Link href={category.href || "/properties"} className="mt-5 inline-flex w-fit items-center gap-2 rounded-full bg-[linear-gradient(100deg,#fa7000_0%,#f90032_62%,#d000c9_100%)] px-4 py-2 text-xs font-bold text-white shadow-[0_5px_12px_rgba(249,0,80,0.20)] transition-transform hover:scale-105 sm:mt-auto">Read More <FaArrowRight /></Link></div>
              </motion.article>
            );
          })}
        </div>
